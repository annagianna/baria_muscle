# Correlations: top-15 pathways (deltaFFMI) and metabolites
# Barbara Verhaar
...
out_dir <- "results/graphs/path_metabolome_correlations"
...
# Baseline (v0) pathway abundance for the top 15
...
#### Join on id, correlate every pathway x metabolite pair ####
shared_ids <- intersect(paths$id, rownames(metab_mat))
cat(length(shared_ids), "participants with both baseline pathway and metabolome data\n")

paths_mat <- paths |>
  filter(id %in% shared_ids) |>
  column_to_rownames("id")
paths_mat <- as.matrix(paths_mat[shared_ids, , drop = FALSE])
metab_mat <- metab_mat[shared_ids, , drop = FALSE]
stopifnot(identical(rownames(paths_mat), rownames(metab_mat)), !anyNA(paths_mat), !anyNA(metab_mat))

cor_results <- expand_grid(pathway = feats, metabolite = colnames(metab_mat)) |>
  mutate(
    test = map2(pathway, metabolite, ~ cor.test(paths_mat[, .x], metab_mat[, .y], method = "spearman", exact = FALSE)),
...
write.csv(cor_results, file.path(out_dir, "pathway_metabolite_correlations.csv"), row.names = FALSE)

sig <- cor_results |> filter(p_fdr < 0.05)
cat(nrow(sig), "/", nrow(cor_results), "pathway-metabolite pairs FDR < 0.05\n")
...
#### Scatter plots: every FDR-significant metabolite, one panel per pathway ####
pathway_labels <- pathway_label(feats)
names(pathway_labels) <- feats

for (feat in unique(sig$pathway)) {
  sig_metabs <- sig |> filter(pathway == feat) |> arrange(p_fdr) |> pull(metabolite)
  ...
  df <- tibble(id = rownames(paths_mat), abundance = paths_mat[, feat]) |>
    ...
    left_join(sig |> filter(pathway == feat) |> select(metabolite, rho, p_fdr), by = "metabolite") |>
  ...
  p <- ggplot(df, aes(x = abundance, y = level)) +
    ...
    labs(
      ...
      x = "log10 pathway abundance (baseline)", y = "Metabolite level (z-scored log10)"
    ) +
    theme_Publication() +
...
#### ComplexHeatmap: pathways x metabolites ####
...
# Build & save one pathways x `metabs` correlation heatmap
plot_corr_heatmap <- function(metabs, file, column_title, col_width, pdf_width_min,
  ...
  fdr_mat <- cor_results |>
    filter(metabolite %in% metabs) |>
    select(pathway, metabolite, p_fdr) |>
    pivot_wider(names_from = metabolite, values_from = p_fdr) |>
    column_to_rownames("pathway") |>
    as.matrix()
  fdr_mat <- fdr_mat[feats, metabs, drop = FALSE]

  paths_active <- rownames(fdr_mat)[apply(fdr_mat < 0.05, 1, any)]
  cat(file, ":", length(paths_active), "/", length(feats), "pathways have >=1 FDR-significant correlation\n")
  rho_mat <- rho_mat[paths_active, , drop = FALSE]
  fdr_mat <- fdr_mat[paths_active, , drop = FALSE]
  ...
  # Metabolite super pathway strip
  ...
  width <- max(pdf_width_min, 3 + ncol(rho_mat) * col_width) + if (transpose) 5.5 else 0
  height <- if (transpose) max(6, 3 + nrow(rho_mat) * row_height) else 10
  left_padding <- if (transpose) 12 else 8
...
cat(length(sig_metabs_all), "distinct metabolites significant for >=1 pathway\n")

if (length(sig_metabs_all) > 0) { # if anything is sig

  # Full heatmap: every metabolite significant for >=1 pathway
  plot_corr_heatmap(
    sig_metabs_all, file.path(out_dir, "pathway_metabolite_heatmap_vertical.pdf"),
    column_title = "All significant metabolites",
    col_width = 0.45, pdf_width_min = 3, column_fontsize = 10,
    transpose = TRUE, row_height = 0.18
  )

  # Compact version: the 20 metabolites correlated with the most pathways
  metab_rank <- sig |>
    group_by(metabolite) |>
  ...
  plot_corr_heatmap(
    sig_metabs_small, file.path(out_dir, "pathway_metabolite_heatmap_top20_vertical.pdf"),
    column_title = sprintf("Top %d metabolites", length(sig_metabs_small)),
    col_width = 0.45, pdf_width_min = 3, column_fontsize = 10,
    transpose = TRUE, row_height = 0.35
  )
} else {
  cat("No pathway has an FDR-significant metabolite correlation - skipping heatmap\n")
}
