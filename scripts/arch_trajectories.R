...
# Data
baria_muscle <- read_rds("data/260130_BARIA_muscle_clinical.RDS")
View(baria_muscle)
...
body_comp_wide_means <- body_comp_summary |> 
  ...
  select(visit, low_asm_v0, weight, ffm, fm, asm, smm) |> 
  pivot_wider(
    id_cols = visit,
    names_from = low_asm_v0,
    values_from = c(weight, ffm, fm, asm, smm),
    names_prefix = "low_"
  ) |> 
  print()
...
body_comp_wide_delta <- body_comp_summary |> 
  ...
  select(visit, low_asm_v0, delta_weight, delta_ffm, delta_fm, delta_asm, delta_smm) |> 
  pivot_wider(
    id_cols = visit, # identifier
    names_from = low_asm_v0,
    values_from = c(delta_weight, delta_ffm, delta_fm, delta_asm, delta_smm),
    names_prefix = "low_"
  ) |> 
  print()
...
p_weight <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_weight = t.test(weight_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_ffm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_ffm = t.test(ffm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_fm <- baria_muscle_trajectories %>%
  ...
  group_by(visit) %>%
  summarise(
    p_fm = t.test(fm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_asm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_asm = round(t.test(asm_kg ~ low_asm_v0)$p.value, 5),
    .groups = "drop"
  )
...
p_smm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_smm = t.test(smm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_delta_weight <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_delta_weight = t.test(delta_weight_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_delta_ffm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_delta_ffm = t.test(delta_ffm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_delta_fm <- baria_muscle_trajectories %>%
  ...
  group_by(visit) %>%
  summarise(
    p_delta_fm = t.test(delta_fm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_delta_asm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_delta_asm = t.test(delta_asm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
p_delta_smm <- baria_muscle_trajectories |> 
  ...
  group_by(visit) |> 
  summarise(
    p_delta_smm = t.test(smm_kg ~ low_asm_v0)$p.value,
    .groups = "drop"
  )
...
body_comp_gt_means_tbl <- body_comp_wide_means_p |>
  ...
  cols_label(
    weight_low_yes = "Low ASM", weight_low_no = "High ASM",
    ffm_low_yes = "Low ASM", ffm_low_no = "High ASM", 
    fm_low_yes = "Low ASM", fm_low_no = "High ASM",
    smm_low_yes = "Low ASM", smm_low_no = "High ASM",
    asm_low_yes = "Low ASM", asm_low_no = "High ASM",
...
ggplot(baria_muscle, aes(y = ffm_kg_v0, color = low_smm_by_weight_v0)) +
  geom_boxplot()
